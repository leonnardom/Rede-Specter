  ...
  async run(message, args, prefix) {
    ...
    if (!USER)
      return message.channel.send(
        `${Emojis.Errado} - ${message.author}, você deve mencionar quem deseja roubar.`
      );
        ...
        if (steal !== null && cooldown - (Date.now() - steal) > 0) {
          return message.channel.send(
            `${Emojis.Cooldown} - ${
              ...
              .replace("minsutos", "minutos")}** para roubar alguém novamente.`
          );
        } else {
          if (user.coins == 0)
            return message.channel.send(
              `${Emojis.Errado} - ${message.author}, este usuário não tem nenhum dinheiro em sua carteira, portanto não é possível roubar ele.`
            );
          ...
          if (luck == 2) {
            ...
            const FAIL = new ClientEmbed(message.author)
              ...
              .setColor("RED")
              .setDescription(
                `Você tentou realizar um roubo e acabou sendo preso.\nVocê perdeu um total de **R$${Util.toAbbrev(
                  amount
                )}**.`
              );
            ...
            await User.findOneAndUpdate(
              ...
              { $set: { coins: author.coins - amount, steal: Date.now() } }
            );
            ...
            const SUCESS = new ClientEmbed(message.author)
              ...
              .setColor("GREEN")
              .setDescription(
                `Você roubou o **${USER}** com sucesso.\nVocê conseguiu um total de **R$${Util.toAbbrev(
                  amount
                )}** neste roubo.`
              );
